...
describe("ResourceSearch", () => {
  ...
  it("calls searchUsers after debounce when typing", async () => {
    mockedSearchUsers.mockResolvedValueOnce([baseResource] as unknown as Awaited<ReturnType<typeof searchUsers>>);
    setup();
  ...
  it("does not show already selected resources in options", async () => {
    ...
    await waitFor(() => {
      ...
      expect(options.find((opt) => opt.textContent === "Projector Room")).toBeUndefined();
    });
  });
  ...
  it("shows loading text when searching", async () => {
    ...
    mockedSearchUsers.mockReturnValueOnce(searchPromise as unknown as ReturnType<typeof searchUsers>);
    setup();
