...
jest.mock("@/components/Attendees/ResourceSearch", () => ({
  ResourceSearch: ({ onChange }: { onChange: (event: null, value: { displayName: string; openpaasId: string }[]) => void }) => (
    <div data-testid="resource-search">
      <button
        data-testid="mock-resource-search-select"
        onClick={() => onChange(null, [{ displayName: "Room A", openpaasId: "room-a-id" }, { displayName: "Room B", openpaasId: "room-b-id" }])}
      >
        Select Resources
      </button>
    </div>
...
const mockedAddCalendarResourceAsync = addCalendarResourceAsync as unknown as jest.Mock;
...
describe("CalendarResources", () => {
  ...
  const setup = (isOpen = true) => {
    ...
    renderWithProviders(
      <CalendarResources onClose={onClose} open={isOpen} />,
      { user: baseUser }
    );
    return { onClose };
  };
  ...
  it("does not render when isOpen is false", () => {
    setup(false);
    expect(screen.queryByText("calendar.browseResources")).not.toBeInTheDocument();
  });
  ...
  it("fetches resource calendars and adds them using Promise.allSettled logic", async () => {
    ...
    mockedGetCalendars.mockImplementation((userId) => {
      ...
      if (userId === "room-b-id") return Promise.reject(new Error("Failed fetching Room B"));
    ...
    const addButton = await screen.findByRole("button", { name: "actions.add" });
